...
class DateFinder(object):
    ...
    DIGITS_PATTERN = '\d+'
    DAYS_PATTERN = 'monday|tuesday|wednesday|thursday|friday|saturday|sunday|mon|tue|tues|wed|thur|thurs|fri|sat|sun|today|yesterday|tomorrow'
    ...
    def find_dates(self, text, source=False, index=False, strict=False, base_date=None, from_future=False):

        for date_string, indices, captures in self.extract_date_strings(text, strict=strict):

            as_dt = self.parse_date_string(date_string, captures, base_date, from_future)
            if as_dt is None:
    ...
    def parse_date_string(self, date_string, captures, base_date=None, from_future=False):
        ...
        as_dt = dateparser.parse(date_string, settings={'PREFER_DATES_FROM': 'future' if from_future else 'past',
...
def find_dates(text, source=False, index=False, strict=False, base_date=None, from_future=False):
    """
    ...
    :type strict: boolean
    :param base_date:
        date to use for relative dates parsing
    :type strict: date
    :param from_future:
        if True future dates will be preferred ovr past ones
    :type strict: boolean
    ...
    return date_finder.find_dates(text, source=source, index=index, strict=strict, base_date=base_date,
